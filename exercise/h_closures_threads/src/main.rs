...
fn main() {
    ...
    // the child threads.
    let (tx_x, rx_x) = channel::unbounded();
    let val_x = 123;

    println!("Main Thread: val for x {}", val_x);
    tx_x.send(val_x).unwrap();

    let handle_x = thread::spawn(move || {
        for msg in rx_x {
            println!("Thread x: Received {}", val_x);
        }
    });

    let (tx_y, rx_y) = channel::unbounded();
    let val_y = 456;

    println!("Main Thread: val for y {}", val_y);
    tx_y.send(val_y).unwrap();

    let handle_y = thread::spawn(move || {
        for msg in rx_y {
            println!("Thread y: Received {}", val_y);
        }
    });
